...
const ImageGallery = ({
  ...
  return <>
      <div ref={galleryRef} className="scroll-mt-24">
        <motion.div layout className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <AnimatePresence mode="popLayout">
            {visibleImages.map((image, index) => <motion.div key={image.src} initial={{
            opacity: 0,
            scale: 0.9,
            y: 20
          }} animate={{
            ...
            duration: 0.4,
            delay: index >= initialCount && isExpanded ? (index - initialCount) * 0.05 : index * 0.03
          }} layout whileHover={{
            scale: 1.05,
            zIndex: 10
          }} onClick={() => openLightbox(index)} className="group relative overflow-hidden rounded-lg shadow-lg cursor-pointer" role="button" tabIndex={0} aria-label={`View ${image.alt} in lightbox`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              openLightbox(index);
            }
          }}>
                <OptimizedImage src={image.src} alt={image.alt} aspectRatio="square" className="transition-transform duration-500 group-hover:scale-110" />
      ...
      <ImageLightbox images={images} currentIndex={currentImageIndex} isOpen={lightboxOpen} onClose={() => setLightboxOpen(false)} onNext={handleNext} onPrevious={handlePrevious} />
    </>;
};
