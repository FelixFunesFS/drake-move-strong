...
export function GoogleMapEmbed({ height = "400px", className = "" }: GoogleMapEmbedProps) {
  ...
  const handleMapInteraction = () => {
    if (!hasFired.current) {
      ...
      trackMetaEvent('FindLocation', { content_name: 'Google Map' });
    }
  };

  return (
    <div className={`rounded-2xl overflow-hidden shadow-lg ${className}`} style={{ minHeight: height }} onClick={handleMapInteraction}>
      <iframe
